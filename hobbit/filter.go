...
func NewFilter(db *Hobbit, coll string) *Filter {
	return &Filter{
		hobbit:      db,
		coll:        coll,
...
func (f *Filter) Insert(values Map) (uint64, error) {
	...
	b, err := f.hobbit.Encoder.Encode(values)
	if err != nil {
		return 0, err
	}
...
func (f *Filter) Update(values Map) ([]Map, error) {
	...
	for _, record := range records {
		...
		b, err := f.hobbit.Encoder.Encode(record)
		if err != nil {
			return nil, err
		}
...
func (f *Filter) findFiltered(bucket *bbolt.Bucket) ([]Map, error) {
	...
	bucket.ForEach(func(k, v []byte) error {
		...
		if err := f.hobbit.Decoder.Decode(v, &record); err != nil {
			return err
		}
		include := true
